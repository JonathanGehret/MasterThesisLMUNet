...
def optimize_one_landscape(target_metric, input_data, model, optimizer, loss_fn, device='cuda'):
    ...
    # Propably one zero_grad enough? Yes, clear this one according to CGPT
    # optimizer.zero_grad()
...
def optimize_one_landscape_fixed_digitization(target_metric, input_data, model, optimizer, loss_fn, accumulation_steps=1, device='cuda', epoch=0):
    model = model.to(device)        
    input_data = input_data.to(device)
    model.eval()
