...
#import torch
'''
def apply_integer_constraints_original(landscape_values, min_class, max_class, mode='clamp'):
...
def apply_integer_constraints(landscape_values, min_class, max_class, mode='clamp'):
    ...
    if mode == 'rand':
        ...
        # Clamp and the values to the desired integer range
...
def apply_integer_constraints_hmm(landscape_values, min_class, max_class, mode='rand'):
...
def optimize_one_landscape(target_metric, input_data, model, optimizer, loss_fn, device='cuda', int_constraints=None):
    ...
    if int_constraints:
        # Apply integer contstraints
        input_data.data = apply_integer_constraints(input_data.data, 0, 1, mode=int_constraints)
